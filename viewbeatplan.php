...
$_SESSION['cit'] = $city;
$_SESSION['count']=1;
$results = mysql_query("SELECT * FROM uploading_beatexcel WHERE zone='$region' AND state='$state' AND city='$city'");
...
if($row_value==0){
    ...
    $results = mysql_query("SELECT * FROM uploading_beatexcel");
}
if(isset($_POST['submit']) && empty($_POST['region']) && empty($_POST['state']) && empty($_POST['city'])){
...
if(!empty($_SESSION['reg']) && !empty($_SESSION['sta']) && !empty($_SESSION['cit'])){
 ...
 $sta = $_SESSION['sta'];
 $cit = $_SESSION['cit'];
$results = mysql_query("SELECT * FROM uploading_beatexcel WHERE zone='$reg' AND state='$sta' AND city='$cit'");
}else{
...
if($_SESSION['count']==1 && $ak==0){
                    ...
                    echo '<table class="table table-bordered table-striped table-condensed" style="background-color: #ffffe5;">';
                    echo '<thead>
        <tr>
          <th>S.no</th>
          <th>Zone</th>
          ...
          <th>DOV 6</th>
                    ...
                    for ($i = $start; $i < $end; $i++) {
                        ...
                        echo '<td>' . mysql_result($results, $i, 'vm town') . '</td>';
                        ...
                        echo '<td>' . mysql_result($results, $i, 'category') . '</td>';
                        echo '<td>' . mysql_result($results, $i, 'address') . '</td>';
                        echo '<td>' . mysql_result($results, $i, 'location') . '</td>';
                        echo '<td>' . mysql_result($results, $i, 'contact') . '</td>';
                        echo '<td>' . mysql_result($results, $i, 'mobile no') . '</td>';
                        echo '<td>' . mysql_result($results, $i, 'dov1') . '</td>';
                        echo '<td>' . mysql_result($results, $i, 'dov2') . '</td>';
                        echo '<td>' . mysql_result($results, $i, 'dov3') . '</td>';
                        echo '<td>' . mysql_result($results, $i, 'dov4') . '</td>';
                        echo '<td>' . mysql_result($results, $i, 'dov5') . '</td>';
                        echo '<td>' . mysql_result($results, $i, 'dov6') . '</td>';
                        echo "</tbody>";
                    }       
                    // close table>
                echo "</table>";
            ...
            ?>
              ...
              <?php
                    ...
                    echo '</ul></div>';
                    echo '<div>';
                    echo '<a href="export_beatplan.php" id="export" type="button" class="btn btn-info">Export</a>';
                    echo '</div>';
}
...
th,td{
      ...
      <footer class="navbar-fixed-bottom site-footer">
          <div class="text-center">
              ...
              <a href="viewbeatplan.php#" class="go-top">
